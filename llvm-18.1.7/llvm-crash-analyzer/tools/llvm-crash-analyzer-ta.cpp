...
//===----------------------------------------------------------------------===//
//
// This file implements the entry point for the llvm-crash-analyzer-ta tool,
// used for testing purposes only. It performs the Taint Analysis on the MIR
// file provided.
//
//===----------------------------------------------------------------------===//
...
static opt<bool> Help("h", desc("Alias for -help"), Hidden,
                      cat(CrashAnalyzerTA));
static opt<std::string> InputFilename(Positional, desc("<input mir file>"),
                                      cat(CrashAnalyzerTA));
static opt<std::string>
    OutputFilename("out-file", cl::init("-"),
                   cl::desc("Redirect output to the specified file"),
                   cl::value_desc("filename"), cat(CrashAnalyzerTA));
...
int main(int argc, char **argv) {
  ...
  if (!isCATargetSupported(TheTriple)) {
    llvm::errs() << "\n Crash Analyzer TA does NOT support target "
                 << TheTriple.getTriple();
    exit(1);
  }
  ...
  if (MIR->parseMachineFunctions(*M, *MMI)) {
    WithColor::error(errs())
        << "unable to parse MIR functions from input file\n";
    exit(1);
  }
  MMI->finalize();
