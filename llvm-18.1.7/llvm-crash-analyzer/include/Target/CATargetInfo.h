...
public:
  ...
  virtual std::optional<unsigned> getID(std::string RegName) const = 0;
  ...
  std::optional<uint64_t> getInstAddr(const MachineInstr* MI) {
  ...
  // Get InstAddr from the InstAddrs map for the MI.
  std::optional<uint64_t> getInstSize(const MachineInstr* MI) {
    if (InstAddrs.count(MI) == 0)
      return std::nullopt;
  ...
  virtual std::optional<std::string> getPC() const = 0;
...
public:
  ...
  std::optional<unsigned> getID(std::string RegName) const override;
  ...
  std::optional<std::string> getPC() const override;
