...
bool X86InstrInfo::isLoad(const MachineInstr &MI) const {
  switch (MI.getOpcode()) {
  case X86::MOV32rm:
  case X86::MOV64rm:
  case X86::MOVSX64rm32:
...
X86InstrInfo::getDestAndSrc(const MachineInstr &MI) const {
  ...
  case X86::MOV32mi: {
    if (getMemOperandWithOffset(MI, BaseOp, Offset, OffsetIsScalable, TRI))
      return DestSourcePair{BaseOp, &MI.getOperand(5), Offset, std::nullopt, ImmVal};

    // This should be scaled indexing addressing mode.
    // Dest: DestReg + (DestScale * DestIndexReg) + DestOff.
    const MachineOperand *Src = &(MI.getOperand(5));
    const MachineOperand *Dst = &(MI.getOperand(0));
    MachineOperand *DstScale =
        const_cast<MachineOperand *>(&(MI.getOperand(1)));
    MachineOperand *DstIndexReg =
        const_cast<MachineOperand *>(&(MI.getOperand(2)));
    // TODO: Is ImmVal needed for Scaled Index mode ? - NO
    // Keep default (zero) DestOff to mark it a memory location.
    return DestSourcePair{Dst,  Src,      0,           std::nullopt, nullptr,
                          std::nullopt, DstScale, DstIndexReg, 0};
  }
  case X86::MOV8rm:
  case X86::MOV32rm:
  case X86::MOV64rm:
  ...
  case X86::XOR64rm: {
    ...
    return DestSourcePair{Dst,     Src,     std::nullopt, 0,        nullptr,    std::nullopt,
  ...
  case X86::ADD64i32: {
    ...
    return DestSourcePair{Dest, Src, std::nullopt, std::nullopt, nullptr, std::nullopt, nullptr, 0, 0};
  }
  case X86::ADD32ri8:
  case X86::ADD64ri8:
  case X86::SUB64ri8:
  ...
  case X86::SUB64ri32: {
    ...
    return DestSourcePair{Dest, Src, std::nullopt, std::nullopt, Src2, std::nullopt, nullptr, 0, 0};
  }
  case X86::LEA64r:
  case X86::LEA32r: {
    const MachineOperand *Dest = &(MI.getOperand(0));
  ...
  case X86::LEA64_32r: {
    ...
    // Keep default SrcOff to mark it a memory location.
    return DestSourcePair{Dest,     Src,     std::nullopt, Offset, nullptr, std::nullopt,
                          nullptr, nullptr, 0,    SrcScale, SrcIndexReg};
  }
  ...
  case X86::CVTSS2SDrr_Int: {
    const MachineOperand *Dest = &(MI.getOperand(0));
    const MachineOperand *Src = &(MI.getOperand(1));
    return DestSourcePair{Dest, Src, std::nullopt, std::nullopt, ImmVal};
  }
  case X86::UCOMISDrr: {
    const MachineOperand *Src = &(MI.getOperand(0));
    const MachineOperand *Source2 = &(MI.getOperand(1));
    return DestSourcePair{nullptr, Src,     std::nullopt,    std::nullopt, Source2,
                          std::nullopt,    nullptr, nullptr, 0};
  }
  case X86::CMP32rm:
  case X86::CMP64rm: {
    const MachineOperand *Src = &(MI.getOperand(0));
    if (!getMemOperandWithOffset(MI, BaseOp, Offset, OffsetIsScalable, TRI))
      return std::nullopt;
    return DestSourcePair{nullptr, Src,     std::nullopt,    std::nullopt, BaseOp,
  ...
  case X86::CMP64mi8: {
    ...
    if (!getMemOperandWithOffset(MI, BaseOp, Offset, OffsetIsScalable, TRI))
      return std::nullopt;
    return DestSourcePair{nullptr, BaseOp,  std::nullopt,    Offset, Src2,
                          std::nullopt,    nullptr, nullptr, 0};
  }
  case X86::CMOV16rm:
  case X86::CMOV32rm:
  ...
  case X86::CMOV64rr: {
    /* FIXME: */
    return DestSourcePair{nullptr, nullptr, std::nullopt,    std::nullopt, nullptr,
                          std::nullopt,    nullptr, nullptr, 0};
  }
  case X86::AND8rr:
  case X86::AND16rr:
  ...
  case X86::TEST32mi: {
    if (!getMemOperandWithOffset(MI, BaseOp, Offset, OffsetIsScalable, TRI))
      return std::nullopt;
    return DestSourcePair{nullptr, BaseOp,  std::nullopt,    Offset, &MI.getOperand(5),
                          std::nullopt,    nullptr, nullptr, 0};
  }
  case X86::TEST8rr:
  case X86::TEST16rr:
  ...
  case X86::BT32rr:
  case X86::BT64rr: {
    return DestSourcePair{nullptr, &MI.getOperand(0), std::nullopt,
                          std::nullopt,    &MI.getOperand(1), std::nullopt,
                          nullptr, nullptr,           0};
  }
  case X86::TEST16i16:
  case X86::TEST16mr:
...
std::optional<RegImmPair> X86InstrInfo::isAddImmediate(const MachineInstr &MI,
                                                  Register Reg) const {
